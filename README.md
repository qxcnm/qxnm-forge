...
这个项目还在早期，但已经不是只能看的空架子：它能连接模型、连续调用工具、保存会话，也能作为本地 daemon 接给以后要做的 React 界面。
...
目前的界面还是纯文本 CLI。React UI、完整 Session 管理、更多 Provider 登录方式以及 MySQL/PostgreSQL adapter 还在后续计划里。
