...
桌面粘贴按同步 `ClipboardEvent.files/items`、异步 `navigator.clipboard.read()`、Tauri 原生图片
兜底的顺序尝试，并跨投影去重。原生权限只允许 `clipboard-manager:allow-read-image`，不得为此开放读文本、写入或清空
剪贴板；RGBA 必须先经尺寸、checked arithmetic 与 512 KiB 编码上限后转换为 PNG。WebKitGTK/
Wayland 产生空 MIME 或前两层空投影时仍可进入原生兜底；`createImageBitmap` 失败时可用受限
`HTMLImageElement` 解码。文档级监听只能在 Composer 或非输入区域接管图片，不能截获设置页等
其他输入框的普通粘贴。

图片可先以本地缩略图进入草稿，发送时才以当前完整模型 identity 的
`capabilities.input` 做 fail-closed 校验。只读文本模型不得调用 `artifacts/create`；上传或
`run/start` 失败必须保留原草稿和附件，只有 run 被服务接受后才清除提交快照。等待上传/接受或
Session 处于 busy 时，添加、删除、粘贴和发送附件都禁用；切换后端或 Session 必须清理草稿附件，
避免跨 Session 误发。模型选择项应分别显示“图片输入”“图片输出”或“仅文本”，但标签不能替代
后端最新 capability 校验。

durable 用户输入图片与模型输出图片都从 `session/get` 的 `image_ref` 重建。只有当前服务广告
`artifacts/read` 时 UI 才能按连续 offset 回读；每块最多 512 KiB、单图最多 32 MiB，并在 WebView
解码前复核 artifact ID、MIME、byteLength、SHA-256、canonical Base64 与 PNG/JPEG/WebP/GIF
魔数。一次 Session 最多加载 32 张、总计 64 MiB、并发 2，预算从最新消息向前分配后仍按消息原序
展示；超额、损坏、缺失或方法未广告时显示失败占位并保留同消息文字。React 只接收经过验证的
`data:` URL，不接收宿主路径或远程图片 URL。成功刷新 durable snapshot 后不得再追加同一条本地
用户消息；快照失败或方法不支持时才可保留本地已接受消息作为降级投影。
...
Codex 的配置。工具、图片输入、图片输出能力分别来自连接的显式 `supportsTools`、
`supportsImageInput`、`supportsImageOutput` 与 daemon 最新广告交集；Image key 缺失时图片输出
不得显示为可用，也不得回退 Responses key，但文字与已声明的图片输入 route 仍可保留。图片输出
有效时 daemon 会暂时把工具广告收窄为 false；这不改写表单中的 `supportsTools`，移除 Image key
并按 `restartRequired` 重启后，UI 必须采用新快照中恢复的原工具声明。
...
`tools + image_input` 的 conformant route；虽然通用图片 artifact 读取与渲染闭环已经实现，UI
仍必须明确显示 Computer Use 实验性未就绪，不能把尺寸/指针文本或普通粘贴图片冒充经审批截图。
展示、安装或启用插件卡片都不能追加工具 ID。
