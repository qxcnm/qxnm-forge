...
中 `enabled=true` 且 `credentialConfigured=true` 的自定义 Provider 所声明模型。该投影不读取
...
待审批状态必须从 `session/get` 返回的 durable `approval.requested` 与后续
...
expiry；损坏的请求必须 fail closed，后续同 ID 的 resolution 必须移除该请求。审批卡的可选
决定只能来自请求自身 `choices` 中的闭合值 `allow_once|allow_session|deny`，不能补全、改写或
扩大授权。只有当前 `initialize.capabilities.methods` 广告 `approval/respond` 时这些按钮才可
提交；未广告、请求过期或正在提交时仍可展示 durable 请求，但必须明确不可响应并禁用操作，
