...
在 `run/start` 之前，客户端可用 `artifacts/create` 把粘贴或选择的受支持图片发布到同一
Session。请求只接受 PNG、JPEG、WebP、GIF，严格 canonical Base64，解码后单张不超过
524288 bytes；服务必须先 durable 发布文件与 `artifact.created`，再返回不含路径和字节的
`artifact` 引用：

```json
{"sessionId":"s-opaque","mediaType":"image/png","dataBase64":"iVBORw0KGgo..."}
```

`dataBase64` 不得进入日志、Session、错误或响应。普通文本文件由客户端解码成 `text` 内容块；
当前协议不宣称任意二进制文件能被模型消费。

客户端重建 durable 图片时只能调用已广告的
`artifacts/read {sessionId,artifactId,offset}`。服务只允许读取该 Session journal 中较早
`artifact.created` 发布、且当前仍由相同普通文件身份承载的 PNG、JPEG、WebP 或 GIF；不得按
客户端路径、URL、display name 或跨 Session ID 查找。单图最多 32 MiB，每次最多返回
512 KiB，每个连接最多保留两个完整验证的连续读取游标，响应严格为
`{artifact,offset,dataBase64,nextOffset}`。`artifact` 元数据在所有分块中不可变化，`offset`
必须回显请求值，非末块的 `nextOffset` 必须精确等于 `offset + decodedLength`，末块为
`null` 且结束位置必须等于 `artifact.byteLength`。零字节分块、越界 offset、非 canonical
Base64、MIME/魔数/长度/SHA-256 不一致、链接/reparse point/device 或缺失文件均失败关闭。
wire、日志与错误不得包含宿主路径；该方法不接受任意 artifact 下载，也不发起网络请求。
...
`run/start.input.content[].image_ref` 必须指向同一 Session 已 durable 发布并重新验证的图片。
只有所选 descriptor 的 `capabilities.input` 包含 `image` 时才能进入 Provider request；否则在
Provider 网络调用前失败关闭。`openai-responses` 把每张图片映射为
`input_image.image_url`，`openai-completions` 映射为 `image_url.url`，两者都只在本次请求
内构造 canonical `data:<mediaType>;base64,...`，不得把 artifact 路径或远程 URL 传给 Provider。
...
- `providerConnections/list {}` 返回 `{connections:[...]}`，每项只有非敏感配置、
  `credentialConfigured` 与 `imageCredentialConfigured`；配置中的 `modelsUrl` 是模型发现的
  精确 URL，`supportsTools`、`supportsImageInput` 与 `supportsImageOutput` 是用户显式能力声明；
...
本地管理边界还定义 `providerCredentials/set {providerId,credentialKind,credential}` 与
`providerCredentials/remove {providerId,credentialKind}`，其中 `credentialKind` 只能是
`responses` 或 `image`；成功只返回
`{providerId,credentialKind,credentialConfigured,restartRequired:true}`。它们不得由通用 WebView 方法
...
图片输入声明只在 Responses credential 可用、连接有效时进入 descriptor；图片输出还必须同时
存在独立 Image credential，否则 `capabilities.output` 只能保留 `text`。Image credential 永不
回退到 Responses credential；缺失 Image credential 不得移除仍可执行的文字或图片输入 route。
有效图片输出还必须把 descriptor `tools` 强制收窄为 false，并让 Agent function-tool 集合为空；
该运行时收窄不修改持久化 `supportsTools`。移除 Image credential 并重启后，图片输出广告消失，
原 `supportsTools` 声明重新生效。有效图片输出的 Responses 请求精确使用 `stream:false` 与
`tools:[{"type":"image_generation"}]`，Chat 请求精确使用 `stream:false` 与
`modalities:["text","image"]`，两者都不得混入 function-tool 定义。对应响应中 Chat 只接受
`choices[0].message.images[].image_url.url` 的 canonical 图片 data URL，Responses 只接受
`output[].type:"image_generation_call"` 的 canonical PNG Base64 `result`。整批最多八张、解码
总计最多 4 MiB；完整 strict JSON、数量、Base64、MIME 与魔数必须全部验证后才能产生图片完成
信号。Responses 非流式对象还必须具有非空 `id`、顶层 `status:"completed"` 与 `output` 数组，
只有完整 completed 对象才产生 `stop`。Chat 只要包含图片，原始 `finish_reason` 就必须精确为
`"stop"`；纯文本完成沿用既有 finish-reason 归一化。远程图片 URL、未知 MIME 或部分有效批次均失败关闭；合法文字可以作为同一图片完成的
说明保留。Provider 正文和图片字节不得进入错误、日志或协议事件。
...
- `models/list {providerId?}` returns only configured, usable models and their
  ...
  Each descriptor includes duplicate-free `capabilities.input` and
  `capabilities.output` media arrays whose first item is `text`; `image` appears
  only after the exact route's input/output gates above are satisfied.
